...
values, and **never** put the Supabase **service role** key anywhere in this
project — only the publishable/anon key belongs in frontend code. The app
validates these variables at startup (`src/lib/supabase.ts`) and throws a
...
`<public_token>/photo`. Public visitors can only read the photo belonging to
an *active* employee, and only via a short-lived signed URL generated
client-side from the token already present in the page URL — no internal
storage path is ever returned by the API.
...
| Employee photo doesn't load on the public page | Confirm the employee is `is_active = true` and the storage policies from `20260714173924_storage_employee_photos.sql` were applied. |
